...
export function ReviewPage() {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <svg key={`full-${i}`} className="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
    ...
    if (hasHalfStar) {
      stars.push(
        <svg key="half" className="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
          <defs>
  ...
  return (
    <div className="w-full flex flex-col items-center bg-black text-white py-12 px-4">
      ...
      <button className="mt-10 flex items-center border border-green-500 text-green-500 px-6 py-2 rounded-md hover:bg-green-500 hover:text-black transition duration-300">
        Read More Reviews
